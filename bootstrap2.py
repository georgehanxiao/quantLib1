...
def get_spot_rates(yieldcurve, day_count, calendar=ql.UnitedStates(), months=121):
    ...
    for yrs in yieldcurve.times():
        ...
        disc_factor = yieldcurve.discount(yrs)
        discountFactors.append(disc_factor)
    return DataFrame(list(zip(tenors, spots, discountFactors)),columns=["Maturities","Curve", "DF"],index=['']*len(tenors))
...
rows = zip(splz.Maturities,splz.Curve, splz.DF)

with open('OISBootstrap2.csv','w',newline='') as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)
